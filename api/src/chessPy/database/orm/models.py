...
class Role(Base):
    ...
    users = relationship('User', secondary=TABLE_USER_ROLES, lazy='noload', back_populates="roles")

user_roles = db.Table(TABLE_USER_ROLES, Base.metadata,
    db.Column('user_id', db.ForeignKey(f'{TABLE_USERS}.id'), index=True, primary_key=True),
    db.Column('role_id',db.ForeignKey(f'{TABLE_ROLES}.id'), index=True, primary_key=True)
)
